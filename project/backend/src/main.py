...
def solve(scenario: dict[str, Any]) -> dict[str, Any]:
    """Run the real UCS agent (agent.py) against the provided scenario.

    Response contract (CONTRATO.md §2): solution_found, total_cost,
    steps[{op, cost, ...}], message.
    """
    data = scenario if scenario else _load_default_scenario()
    return agent_solve(data)
